...
df_b['team_1'] = game_info[0]
df_b['team_2'] = game_info[1]

team_2 = df_b['team_2'].str.split(' ', n=1, expand=True)
df_b['team_2'] = team_2[0]
df_b.drop(columns=['Game Info'], inplace=True)

df_b['Opp'] = None

for i in df_b.index:
    if df_b['TmAbb'][i] == df_b['team_1'][i]:
        df_b['Opp'][i] = df_b['team_2'][i]
    else:
        df_b['Opp'][i] = df_b['team_1'][i]
...
df_b.drop(columns=['team_1','team_2'], inplace=True)
...
# proj_hits = (df_b['H'] + df_b['2B'] + df_b['3B']) * df_b['h_fac']
# proj_hr = df_b['HR'] * df_b['hr_fac']
# proj_r = (df_b['R'] + df_b['RBI']) * df_b['r_fac']
# proj_bb = df_b['BB'] * df_b['bb_fac']
...
# df_p['team_1'] = game_info[0]
# df_p['team_2'] = game_info[1]

# team_2 = df_p['team_2'].str.split(' ', n=1, expand=True)
# df_p['team_2'] = team_2[0]
# df_p.drop(columns=['Game Info'], inplace=True)

# df_p['Opp'] = None

# for i in df_p.index:
#     if df_p['TmAbb'][i] == df_p['team_1'][i]:
#         df_p['Opp'][i] = df_p['team_2'][i]
#     else:
#         df_p['Opp'][i] = df_p['team_1'][i]
...
# df_p.drop(columns=['team_1','team_2'], inplace=True)
...
# proj_hits = df_p['H'] * df_p['h_fac']
# proj_so = df_p['SO'] * df_p['so_fac']
# proj_r = df_p['ER'] * df_p['r_fac']
# proj_bb = df_p['BB'] * df_p['bb_fac']
...
# df_proj.to_csv('../Data/' + date + '/' + slate + '/projections_a.csv')
# #
